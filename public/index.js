(() => {
  ...
  // src/js/modules/languageSwitch.js
  var STORAGE_KEY = "trigal-lang";
  var DEFAULT_LANG = "es";
  var bundle = null;
  ...
  var translate = (lang, path) => {
    var _a;
    const data = readBundle();
    if (!data || !path) return void 0;
    const [source, ...rest] = path.split(".");
    return getByPath((_a = data[source]) == null ? void 0 : _a[lang], rest.join("."));
  };
  var applyText = (lang) => {
  ...
  var applyDocumentMeta = (lang) => {
    ...
    const ogImageAlt = translate(lang, `${metaRoot}.ogImageAlt`) || translate(lang, "site.meta.ogImageAlt");
  ...
  var t = (path) => translate(currentLang, path);
  var onLanguageChange = (listener) => {
    listeners.push(listener);
  };
  var languageSwitch_default = languageSwitch;

  // src/js/modules/siteHeader.js
  var getFocusable = (root) => [...root.querySelectorAll("a[href], button:not([disabled])")].filter(
  ...
  var siteHeader = () => {
    document.querySelectorAll(".site-header").forEach((root) => {
      ...
      const label = toggle == null ? void 0 : toggle.querySelector(".visually-hidden");
  ...
  // src/js/modules/serviciosCatalog.js
  var DESKTOP_QUERY = "(width >= 960px)";
  var getButtons = (section) => [...section.querySelectorAll(".servicios-catalog__item-button")];
  var isDesktop = () => window.matchMedia(DESKTOP_QUERY).matches;
  var setExpanded = (button, expanded) => {
    const panelId = button.getAttribute("aria-controls");
    const panel = panelId ? document.getElementById(panelId) : null;
    button.setAttribute("aria-expanded", expanded ? "true" : "false");
    if (panel) panel.hidden = !expanded;
  };
  var closeOthers = (section, current) => {
    getButtons(section).forEach((button) => {
      if (button !== current) setExpanded(button, false);
    });
  };
  var serviciosCatalog = () => {
      ...
      root.querySelectorAll(".servicios-catalog__section").forEach((section) => {
        ...
        buttons.forEach((button, index) => {
          ...
          button.addEventListener("keydown", (event) => {
            ...
            (_a = buttons[nextIndex]) == null ? void 0 : _a.focus();
          });
        });
      });
      if ("IntersectionObserver" in window) {
        ...
        const observer = new IntersectionObserver(
          (entries) => {
            var _a;
            const visible = entries.filter((entry) => entry.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0];
            if ((_a = visible == null ? void 0 : visible.target) == null ? void 0 : _a.id) setCurrentIndex(visible.target.id);
          },
          {
  ...
  var serviciosCatalog_default = serviciosCatalog;

  // src/js/modules/contactContext.js
  var WHATSAPP_SELECTOR = '.contact a[href*="wa.me"], .floating-contact a[href*="wa.me"]';
  var applyWhatsappContext = () => {
  ...
  var contactContext_default = contactContext;
  ...
  // src/js/modules/floatingContact.js
  var getFocusable2 = (root) => [...root.querySelectorAll("a[href], button:not([disabled])")].filter(
  ...
  var floatingContact = () => {
    document.querySelectorAll(".floating-contact").forEach((root) => {
      ...
      const label = toggle == null ? void 0 : toggle.querySelector(".floating-contact__toggle-text");
      ...
      const open = () => {
        ...
        panel.hidden = false;
        setExpandedLabel(true);
        const focusable = getFocusable2(panel);
        if (focusable[0]) focusable[0].focus();
      };
      const isOpen = () => toggle.getAttribute("aria-expanded") === "true";
      toggle.addEventListener("click", () => {
      ...
      document.addEventListener("keydown", (event) => {
        ...
        if (event.key !== "Tab" || !isOpen()) return;
        const focusable = [toggle, ...getFocusable2(panel)];
        const first = focusable[0];
        const last = focusable[focusable.length - 1];
        if (event.shiftKey && document.activeElement === first) {
          event.preventDefault();
  ...
  var floatingContact_default = floatingContact;

  // src/js/modules/articleGridSlider.js
  var getDesktopMedia = () => {
  ...
  var applyNavLabels = (root, instance) => {
  ...
  var articleGridSlider_default = articleGridSlider;

  // src/js/index.js
  var initComponents = () => {
    languageSwitch_default();
    siteHeader_default();
    faq_default();
    services_default();
    serviciosCatalog_default();
    contactContext_default();
    smoothScroll_default();
    floatingContact_default();
    articleGridSlider_default();
